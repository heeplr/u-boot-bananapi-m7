...
struct rk_gmac_ops {
	...
	void (*set_clock_selection)(struct gmac_rockchip_plat *pdata);
...
static int rk3588_set_rgmii_speed(struct dw_eth_dev *priv)
{
	...
	switch (priv->phydev->speed) {
	case 10:
		if (priv->phydev->interface == PHY_INTERFACE_MODE_RMII)
...
static int rk3568_set_rgmii_speed(struct dw_eth_dev *priv)
{
	struct clk clk_speed;
	int ret;
...
static void rk3568_set_to_rmii(struct gmac_rockchip_plat *pdata)
{
...
static void rk3568_set_to_rgmii(struct gmac_rockchip_plat *pdata)
{
	struct rk3568_grf *grf;
	void *con0, *con1;
...
static void rk3588_set_to_rmii(struct gmac_rockchip_plat *pdata)
{
...
static void rk3588_set_to_rgmii(struct gmac_rockchip_plat *pdata)
{
...
static void rk3588_set_clock_selection(struct gmac_rockchip_plat *pdata)
{
	struct rk3588_php_grf *php_grf;
	...
	php_grf = syscon_get_first_range(ROCKCHIP_SYSCON_PHP_GRF);
	val = pdata->clock_input ? RK3588_GMAC_CLK_SELECT_IO :
...
printf("probing 2\n");
	...
	ret = clk_get_by_name(dev, "pclk_mac", &pclk);
	if (ret) return ret;
clk_set_rate(&pclk, 1);

//	ret = clk_get_by_name(dev, "clk_mac_ref", &pclk);
//	if (ret) return ret;
...
printf("probing 2.5 type interface = %d\n", eth_pdata->phy_interface);
...
const struct rk_gmac_ops rk3568_gmac_ops = {
	.fix_mac_speed = rk3568_set_rgmii_speed,
