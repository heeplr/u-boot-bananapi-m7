...
static int eqos_probe_resources_rk(struct udevice *dev)
{
	...
	if (ret) {
		pr_err("clk_get_by_name(pclk_mac) failed: %d", ret);
		goto err_free_gpio_phy_reset;
	}

	ret = clk_get_by_name(dev, "stmmac_eth", &eqos->clk_master_bus);
	if (ret) {
		pr_err("clk_get_by_name(stmmac_eth) failed: %d", ret);
		goto err_free_clk_slave_bus;
	}

	ret = clk_get_by_name(dev, "aclk_mac", &eqos->clk_ck);
	if (ret) {
		pr_err("clk_get_by_name(aclk_mac) failed: %d", ret);
	...
	if (ret) {
		pr_err("clk_get_by_name(clk_mac_ref) failed: %d", ret);
