...
static int rk3588_clk_scmi_bind(struct udevice *dev)
{
#ifdef CONFIG_SPL_BUILD
	ofnode node, child;
	const char *name;
	...
	ofnode_for_each_subnode(child, node) {
		...
		if (ofnode_read_u32(child, "reg", &protocol_id))
			continue;
		...
		return device_bind_driver_to_node(dev, "rockchip_scmi_clk",
						  name, child, NULL);
	}
#endif
	return 0;
...
static ulong rk3588_clk_scmi_set_rate(struct clk *clk, ulong rate)
{
	...
	return rk3588_clk_scmi_get_rate(clk);
}

static const struct clk_ops rk3588_scmi_clk_ops = {
	.get_rate = rk3588_clk_scmi_get_rate,
	.set_rate = rk3588_clk_scmi_set_rate,
};

U_BOOT_DRIVER(rockchip_scmi_clk) = {
	.name = "rockchip_scmi_clk",
	.id = UCLASS_CLK,
	.ops = &rk3588_scmi_clk_ops,
};
#endif
